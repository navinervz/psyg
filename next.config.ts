...
const securityHeaders = [
  /*
    حالا اجباری. قبلش `-Report-Only` بود و تا وقتی ثابت نشده بود چیزی
    را نمی‌شکند، عوض نشد.

    چطور ثابت شد: کنسول مرورگر روی چهار مسیر رندر مختلف — صفحه‌ی اصلی،
    فرصت‌ها، صفحه‌ی محصول و مقاله‌ی مجله — بعد از لود کامل خوانده شد و
    هر چهار تا صفر تخلف داشتند.

    و مهم‌تر: خودِ ابزار خواندن کنسول هم با یک پیام آزمایشی سنجیده شد.
    «هیچ خطایی نیست» و «هیچ چیزی را نمی‌بینم» دو چیز کاملاً متفاوت‌اند
    که یک شکل به نظر می‌رسند — امروز سه بار همین اشتباه نزدیک بود
    تکرار شود.
  */
...
const nextConfig: NextConfig = {
  ...
  images: {
    ...
    /*
      کش مرورگر برای تصویر بهینه‌شده.

      پیش‌فرض Next شصت ثانیه است — یعنی کاربری که بین صفحه‌ها می‌گردد،
      همان عکس‌ها را بارها می‌گیرد. عکس محصول با آدرس هش‌دار می‌آید و
      عملاً هیچ‌وقت عوض نمی‌شود؛ اگر محصول عکسش را عوض کند، آدرسش هم
      عوض می‌شود.
    */
  ...
  /*
    ─────────────────────────────────────────────────────────────────────
    تجزیه‌ی هوشمند ایمپورت‌های آیکون
    ─────────────────────────────────────────────────────────────────────
    `import { Search, X } from "lucide-react"` در حالت عادی کل ماژول
    فهرست را وارد گراف می‌کند و باندلر باید هزار آیکون را تجزیه کند تا
    بفهمد دو تا لازم است. این هم زمان بیلد را بالا می‌برد هم گاهی
    آیکون‌های استفاده‌نشده در باندل می‌مانند.

    این گزینه ایمپورت را به مسیر مستقیم هر آیکون بازنویسی می‌کند. هیچ
    چیزی حذف نمی‌شود — فقط آنچه لازم نیست از اول وارد نمی‌شود.
  */
  experimental: {
