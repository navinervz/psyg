...
export type WidgetSource = {
  id: string;
  /**
   * دسته‌ی محصولات این ویجت.
   *
   * `"auto"` یعنی ویجت ترکیبی است و دسته از عنوان هر محصول درمی‌آید —
   * برای ویجت دستی‌ای که هم کنسول دارد هم هدفون. بقیه‌ی ویجت‌ها دسته‌ی
   * صریح دارند و از عنوان حدس زده نمی‌شود.
   */
...
/**
 * واژه‌هایی که وقتی *اول* عنوان بیایند، یعنی محصول لوازم جانبی است.
 *
 * ─────────────────────────────────────────────────────────────────────
 * چرا این لازم شد
 * ─────────────────────────────────────────────────────────────────────
 * فرض اولیه‌ی `WIDGET_SOURCES` این بود که هر ویجت دسته‌اش خالص است، پس
 * «از روی اسم محصول حدس نمی‌زنیم». داده‌ی واقعی این فرض را رد کرد:
 * ویجت «کنسول بازی» ده قلم دارد و هر ده تا کیف حمل، روکش آنالوگ و پایه
 * شارژ است. یعنی کاربری که روی «کنسول بازی» می‌زد، انتظار PS5 داشت و
 * کیف می‌دید.
 *
 * ─────────────────────────────────────────────────────────────────────
 * چرا فقط اول عنوان
 * ─────────────────────────────────────────────────────────────────────
 * این نکته‌ی کل ماجراست. عنوان فارسی اسم اصلی را اول می‌آورد و اقلام
 * همراه را آخر:
 *
 *   «گوشی موبایل سامسونگ Galaxy A37 … به همراه یک عدد کاور سیلیکونی
 *    و یک عدد محافظ صفحه نمایش»
 *
 * جستجوی «کاور» در کل عنوان، این گوشی را هم لوازم جانبی می‌کرد — یعنی
 * دقیقاً برعکس چیزی که می‌خواهیم. پس فقط اولین واژه سنجیده می‌شود.
 *
 * و این نگاشت یک‌طرفه است: چیزی را به لوازم جانبی *تنزل* می‌دهد، هیچ‌وقت
 * چیزی را به دسته‌ی گران‌تر ارتقا نمی‌دهد. اشتباهش در بدترین حالت یعنی
 * یک قلم در لوازم جانبی بنشیند، نه اینکه یک کیف به‌عنوان کنسول جا بزند.
 */
...
/** دسته‌هایی که «دستگاه» هستند و لوازم جانبی نباید در آن‌ها بنشیند */
const DEVICE_CATEGORIES = new Set<CategoryId>([
  "mobile",
  "laptop",
  "tablet",
  "console",
...
/**
 * تشخیص دسته از روی *ابتدای* عنوان.
 *
 * ─────────────────────────────────────────────────────────────────────
 * چرا لازم شد
 * ─────────────────────────────────────────────────────────────────────
 * ویجت‌های آماده‌ی افیلیو قابل ویرایش نبودند و «کنسول بازی» فقط کیف حمل
 * داشت. راه‌حل این است که یک ویجت دستیِ ترکیبی ساخته شود — ولی آن ویجت
 * دیگر یک دسته ندارد: هم PS5 دارد هم هدفون.
 *
 * پس برای همان یک ویجت، دسته از عنوان درمی‌آید. بقیه‌ی ویجت‌ها دست
 * نمی‌خورند و همان دسته‌ی اعلام‌شده‌شان را نگه می‌دارند.
 *
 * ─────────────────────────────────────────────────────────────────────
 * چرا فقط ابتدای عنوان، باز هم
 * ─────────────────────────────────────────────────────────────────────
 * همان دلیل لوازم جانبی. «کیف حمل کنسول ps5» واژه‌ی «کنسول» را دارد ولی
 * کنسول نیست. لنگر `^` یعنی فقط چیزی که عنوان *با آن شروع می‌شود* ملاک
 * است — و لوازم جانبی هم اول سنجیده می‌شوند تا هیچ کیفی کنسول نشود.
 */
const TITLE_CATEGORIES: { id: CategoryId; test: RegExp }[] = [
  { id: "console", test: /^(کنسول|پلی\s*استیشن|ایکس\s*باکس|نینتندو)/ },
  { id: "headphone", test: /^(هدفون|هدست|هندزفری|ایرپاد|ایربادز)/ },
  { id: "mobile", test: /^(گوشی|موبایل)/ },
  { id: "laptop", test: /^(لپ\s*تاپ|مک\s*بوک|نوت\s*بوک|اولترابوک)/ },
  { id: "tablet", test: /^(تبلت|آیپد)/ },
  { id: "wearable", test: /^(ساعت|مچ\s*بند|اسمارت\s*واچ)/ },
];

/** آیا عنوان با یکی از واژه‌های لوازم جانبی شروع می‌شود */
function startsAsAccessory(title: string): boolean {
  const head = title.trim().split(/\s+/)[0]?.replace(/[‌]/g, "") ?? "";
  return ACCESSORY_HEADS.has(head);
}

export function classifyByTitle(title: string): CategoryId {
  /*
    لوازم جانبی اول. اگر این بعد از دستگاه‌ها می‌آمد، «دسته بازی» به
    خاطر واژه‌ی «دسته» هیچ‌وقت گرفته نمی‌شد ولی چیزهای دیگری می‌شدند.
  */
  if (startsAsAccessory(title)) return "accessory";

  const clean = title.trim().replace(/[‌]/g, " ");
  const hit = TITLE_CATEGORIES.find((c) => c.test.test(clean));

  /*
    نشناختیم؟ لوازم جانبی.

    حدس زدن دسته‌ی گران‌تر برای چیزی که نمی‌شناسیم، همان اشتباهی است که
    اول ماجرا کیف حمل را کنسول کرد.
  */
  return hit ? hit.id : "accessory";
}

export function resolveCategory(
  title: string,
  declared: CategoryId | "auto",
): CategoryId {
  if (declared === "auto") return classifyByTitle(title);
  if (!DEVICE_CATEGORIES.has(declared)) return declared;
  return startsAsAccessory(title) ? "accessory" : declared;
}
...
export function toProduct(
  ...
  */
  ...
  /*
    ─────────────────────────────────────────────────────────────────────
    درصد تخفیف از خود قیمت‌ها حساب می‌شود، نه از فیلد افیلیو
    ─────────────────────────────────────────────────────────────────────
    نسخه‌ی قبلی به `raw.discount_percent` اعتماد می‌کرد و همان را با سقف
    ۷۰٪ می‌سنجید. روی سایت زنده معلوم شد این فیلد با قیمت‌های خودِ همان
    رکورد نمی‌خواند.

    نمونه‌ی واقعی: «تبلت ۱۰ اینچ یومیو ۵۱۲ گیگابایت رم ۸ گیگابایت» با
    قیمت ۳۱۰٬۰۰۰ تومان روی صفحه‌ی اصلی به‌عنوان «بیشترین افت این هفته،
    ۹۷٪ کمتر» تبلیغ می‌شد. سقف ۷۰٪ نگرفتش، چون `discount_percent` عددی
    زیر ۷۰ گزارش کرده بود در حالی که نسبت واقعی قیمت‌ها ۹۷٪ بود.

    درس: نگهبانی که فیلدی غیر از همان چیزی که محافظت می‌کند را می‌سنجد،
    نگهبان نیست. حالا نسبت واقعی حساب می‌شود.
  */
  const claimedDiscount =
    raw.price > 0 && raw.price_discount > 0
      ? (1 - raw.price_discount / raw.price) * 100
      : 0;

  const hasDiscount =
    raw.price_discount > 0 &&
    raw.price_discount < raw.price &&
    claimedDiscount <= MAX_BELIEVABLE_DISCOUNT;
  ...
  return {
    ...
    image: raw.image_url,
    store: source.store,
    category: resolveCategory(raw.name, source.category),
    ...
    /*
      ─────────────────────────────────────────────────────────────────
      چرا «قیمت قبلی» برابر قیمت امروز است و نه `raw.price`
      ─────────────────────────────────────────────────────────────────
      `raw.price` قیمت پیش از تخفیفِ *فروشگاه* است، نه قیمتی که ما
      قبلاً ثبت کرده باشیم. گذاشتنش در `previousPrice` یعنی سایت روز
      اول ادعا کند «این هفته ارزان شد» — درباره‌ی محصولی که فقط یک بار
      دیده.

      همین شد که «۴ مدل تبلت این هفته ارزان‌تر شدن، ۷۵٪ کمتر» روی
      صفحه‌ی اصلی نشست؛ آن ۷۵٪ تخفیف فروشگاه بود، نه افت قیمت.

      `mergeCatalog` این مقدار را از تاریخچه‌ی واقعی پر می‌کند. تا وقتی
      تاریخچه‌ای نیست، درست‌ترین حرف این است که تغییری نمی‌دانیم — و
      نتیجه‌اش این است که چنین محصولی اصلاً در فهرست «بیشترین افت»
      نمی‌آید.

      هزینه‌اش را می‌پذیریم: وقتی کاتالوگ تازه است، پنل پیشنهادها
      خلوت می‌شود. پنل خلوت از پنل دروغ‌گو بهتر است.
    */
